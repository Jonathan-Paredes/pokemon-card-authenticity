...
Several misclassified cards were identified during external evaluation, including:
...
During analysis, different behaviors were observed.

In some predictions, the model's attention concentrated around the central Pokémon artwork.

In other cases, attention shifted toward regions containing card characteristics and printed details.

For example, one real card was incorrectly classified as fake with approximately:
...
while a counterfeit example was classified as:
...
These results demonstrate why interpretability is important.

A high-confidence prediction does not necessarily mean that the model is using the features that a human expert would consider relevant.

---

# ⚠️ Potential Biases and Shortcuts
...
These characteristics could accidentally become predictive.
...
The dataset contains holographic examples, including different visual characteristics such as:
...
The repository is organized around reproducibility and separation of responsibilities.
